...
function CommentSection({ comment, post, type, refetchComments }: Props) {
  ...
  const [textArea, setTextArea] = useState<string>(
    !isEmpty(comment?.content) ? renderCommentText(comment?.content) : ""
  );
            ...
            getDiffTime(comment?.createdAt) < 15 && (
              <div className="flex space-x-2">
                <div
                  ...
                  className="flex items-center justify-center bg-gray-50 dark:bg-lightgray dark:group-hover:bg-darkgray h-fit rounded-md p-1"
                >
        ...
        <div className="w-full">
          <div className="flex flex-col items-start justify-start">
            {!isEmpty(comment?.content) && (
              <p className="pt-4 text-sm break-words whitespace-pre-line w-fit max-w-full">
