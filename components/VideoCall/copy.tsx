import {
  ...
  function VideoCall() {
    ...
    const handleMessageFromPeer = async (message: any) => {
      ...
      if (data.type === "offer" && !peerConnection.remoteDescription) {
        console.log(
          "videocall",
          "enter offer",
          data,
      ...
      if (data.type === "candidate") {
        ...
        if (!peerConnection || !peerConnection.remoteDescription?.type) {
          console.log(
            "videocall Remote description not set. Cannot add ICE candidate.", peerConnection
          );
    ...
    const [channel] = useChannel(
      ...
      `${room_id}`,
       (message) => {
        ...
        // setParticipants(participantsRef.current);
        // setStream(stream+1);
        console.log(participants, "participants change");
      }
    );
    ...
    return (
      <>
        ...
        <div id="videos">
          {!isEmpty(participants) && (!stream || stream )&& (
            <>
