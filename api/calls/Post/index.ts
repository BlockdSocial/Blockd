...
async function searchPosts(fields: any) {
  return apiCall(
    "searchPost",
    "POST",
    `${endpoints.search}/${endpoints.posts}`,
    fields
  );
}
...
async function fetchUnauthPosts(fields: any) {
  return apiCall("fetchUnauthPosts", "POST", "public/posts/unauth", fields);
}
